...
fn create_router_with_openapi(config: &TestaustimeConfig) -> (Router, utoipa::openapi::OpenApi) {
    ...
    let ratelimiter = Arc::new(
        RateLimiter::keyed(Quota::per_minute(
            NonZeroU32::new(config.max_requests_per_min).expect("Ratelimit set to 0"),
        ))
        .with_middleware(),
    );
    ...
    let register_ratelimiter = Arc::new(
        RateLimiter::keyed(Quota::per_hour(
            NonZeroU32::new(config.max_registers_per_hour).expect("Ratelimit set to 0"),
        ))
        .with_middleware(),
    );
...
async fn main() {
    ...
    let listener = tokio::net::TcpListener::bind(&config.address)
        .await
        .unwrap_or_else(|_| panic!("Could not bind to address {}", &config.address));
    ...
    .await
    .expect("BUG: Could not start server");
}
